...
sleep(1);
if ($idthiscontact || count($idthiscontact)){
$method='v4/leads';
$arParams='filter[pipe][2094802][]=30157465&with=contacts';  
...
for ($k=0; $k<count($idthiscontact);$k++){
    if ($idthiscontact[$k]==$primary['_embedded']['leads'][$j]['_embedded']['contacts'][0]['id']){
        $thisidlead=$primary['_embedded']['leads'][$j]['id'];
...
$method='v4/contacts/'.$idthiscontact;
 ...
 for ($j=0; $j<count($primary ['_embedded']['leads']);$j++){
    if ($idthiscontact==$primary['_embedded']['leads'][$j]['_embedded']['contacts'][0]['id']){
        $thisidlead=$primary['_embedded']['leads'][$j]['_embedded']['leads'][0]['id'];
        break;
    }
}   
}
